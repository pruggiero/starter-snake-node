...
const randomMovement = (possibleMovements, snake, food) => {
  ...
  if (snake.body.length <= 8) {
    ...
    if (possibleMovements.indexOf('left') !== -1 && location[0].x > 5) {
      movement = 'left';
    } 
    if (possibleMovements.indexOf('right') !== -1 && location[0].x <= 6) {
      movement = 'right';
    }
  }

  if (food.length > 0 && ((snake.health < 80 && snake.body.length <= 11) || snake.health < 30) ) {
